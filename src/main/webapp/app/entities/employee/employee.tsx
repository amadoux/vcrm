  ...
  return (
    <div>
      <h2 id="employee-heading" data-cy="EmployeeHeading">
        <Translate contentKey="vcrmApp.employee.home.title">Employees</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="vcrmApp.employee.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/employee/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="vcrmApp.employee.home.createLabel">Create new Employee</Translate>
          </Link>
        </div>
      </h2>
          ...
          {employeeList && employeeList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="vcrmApp.employee.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  <th className="hand" onClick={sort('firstName')}>
                    <Translate contentKey="vcrmApp.employee.firstName">First Name</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('firstName')} />
                  </th>
                  <th className="hand" onClick={sort('lastName')}>
                    <Translate contentKey="vcrmApp.employee.lastName">Last Name</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('lastName')} />
                  </th>
                  <th className="hand" onClick={sort('email')}>
                    <Translate contentKey="vcrmApp.employee.email">Email</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('email')} />
                  </th>
                  <th className="hand" onClick={sort('phoneNumber')}>
                    <Translate contentKey="vcrmApp.employee.phoneNumber">Phone Number</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('phoneNumber')} />
                  </th>
                  <th className="hand" onClick={sort('identityCard')}>
                    <Translate contentKey="vcrmApp.employee.identityCard">Identity Card</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('identityCard')} />
                  </th>
                  <th className="hand" onClick={sort('dateInspiration')}>
                    <Translate contentKey="vcrmApp.employee.dateInspiration">Date Inspiration</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('dateInspiration')} />
                  </th>
                  <th className="hand" onClick={sort('nationality')}>
                    <Translate contentKey="vcrmApp.employee.nationality">Nationality</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('nationality')} />
                  </th>
                  <th className="hand" onClick={sort('uploadIdentityCard')}>
                    <Translate contentKey="vcrmApp.employee.uploadIdentityCard">Upload Identity Card</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('uploadIdentityCard')} />
                  </th>
                  <th className="hand" onClick={sort('companyName')}>
                    <Translate contentKey="vcrmApp.employee.companyName">Company Name</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('companyName')} />
                  </th>
                  <th className="hand" onClick={sort('typeEmployed')}>
                    <Translate contentKey="vcrmApp.employee.typeEmployed">Type Employed</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('typeEmployed')} />
                  </th>
                  <th className="hand" onClick={sort('hireDate')}>
                    <Translate contentKey="vcrmApp.employee.hireDate">Hire Date</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('hireDate')} />
                  </th>
                  <th className="hand" onClick={sort('salary')}>
                    <Translate contentKey="vcrmApp.employee.salary">Salary</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('salary')} />
                  </th>
                  <th className="hand" onClick={sort('commissionPct')}>
                    <Translate contentKey="vcrmApp.employee.commissionPct">Commission Pct</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('commissionPct')} />
                  </th>
                  <th>
                    <Translate contentKey="vcrmApp.employee.manager">Manager</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="vcrmApp.employee.department">Department</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {employeeList.map((employee, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>
                      <Translate contentKey={`vcrmApp.Pays.${employee.nationality}`} />
                    </td>
                    <td>
                    ...
                    <td>
                      <Translate contentKey={`vcrmApp.TypeEmployed.${employee.typeEmployed}`} />
                    </td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="vcrmApp.employee.home.notFound">No Employees found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
